...
import tcdx.uap.constant.AreaTypeEnum;
...
import static tcdx.uap.common.utils.collection.CollectionUtils.findFirst;
...
public class AreaUtils {
    ...
    private AreaUtils() {
        ...
        areas.put(Area.ID_GLOBAL, new Area(Area.ID_GLOBAL, "全球", 0,
                null, new ArrayList<>()));
        // 从 csv 中加载数据
        List<CsvRow> rows = CsvUtil.getReader().read(ResourceUtil.getUtf8Reader("area/area.csv")).getRows();
        ...
        for (CsvRow row : rows) {
            ...
            Assert.isTrue(area != parent, "{}:父子节点相同", area.getLabel());
    ...
    public void InitAll(){
        System.out.println("启动加载 AreaUtils 成功");
    }
    ...
    public static Area parseArea(String pathStr) {
        ...
        for (String path : paths) {
            if (area == null) {
                area = findFirst(areas.values(), item -> item.getLabel().equals(path));
            } else {
                area = findFirst(area.getChildren(), item -> item.getLabel().equals(path));
            }
        }
        return area;
    }
    ...
    private static void getAreaNodePathList(Area node, String path, List<String> paths) {
        ...
        // 构建当前节点的路径
        String currentPath = path.isEmpty() ? node.getLabel() : path + "/" + node.getLabel();
    ...
    public static String format(Integer id, String separator) {
        ...
        for (int i = 0; i < AreaTypeEnum.values().length; i++) { // 避免死循环
            sb.insert(0, area.getLabel());
            // “递归”父节点
            area = area.getParent();
            if (area == null
                    || Objects.equals(area.getValue(), Area.ID_GLOBAL)
                    || Objects.equals(area.getValue(), Area.ID_CHINA)) { // 跳过父节点为中国的情况
                break;
            }
    ...
    public static Integer getParentIdByType(Integer id, @NonNull AreaTypeEnum type) {
        for (int i = 0; i < Byte.MAX_VALUE; i++) {
            ...
            // 情况二：找到根节点，返回空
            if (area.getParent() == null || area.getParent().getValue() == null) {
                return null;
            }
            // 其它：继续向上查找
            id = area.getParent().getValue();
        }
        return null;
    }
